...
const acli_jira_workitem_search_fixture = @embedFile("fixture_acli_jira_workitem_search");
const acli_jira_workitem_view_fixture = @embedFile("fixture_acli_jira_workitem_view");
const acli_jira_workitem_fields_fixture = @embedFile("fixture_acli_jira_workitem_fields");
const acli_confluence_page_view_fixture = @embedFile("fixture_acli_confluence_page_view");
const acli_confluence_space_list_fixture = @embedFile("fixture_acli_confluence_space_list");
const acli_error_stderr_fixture = @embedFile("fixture_acli_error_stderr");
...
test "wrapper: stats record agent-visible stdout and stderr bytes" {
    ...
    try std.testing.expect(std.mem.find(u8, stats_json, "\"v\":3") != null);
...
test "wrapper: failed output is never persisted" {
    const allocator = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const home_path = try tmp.dir.realPathFileAlloc(std.testing.io, ".", allocator);
    ...
    try std.testing.expect(std.mem.find(u8, stats_json, "\"displayed_bytes\":8") != null);
    try std.testing.expect(std.mem.find(u8, stats_json, "\"diagnostic_bytes\":0") != null);
    try std.testing.expect(std.mem.find(u8, stats_json, "\"formatting_saved_bytes\":0") != null);
}

test "wrapper: analytics remain invisible while the shared state lock is held" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    var env = try makeWrapperEnv(allocator, bin_path, home_path);
    defer env.deinit();
    ...
    // post-child persistence boundary.
    ...
    try std.testing.expectEqualStrings("failure sentinel\n", wrapped.stdout);
...
test "sessions: concurrent failures never persist output and preserve analytics" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    try std.testing.expectEqualStrings("CLAUDE-FAILURE-RAW-OUT\n", results[0].stdout);
    ...
    try std.testing.expectEqualStrings("OPENCODE-FAILURE-RAW-OUT\n", results[1].stdout);
...
test "reset ordering: ordinary reset preserves legacy tee and setup state" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "reset ordering: reset all before failing finalization recreates analytics only" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    try std.testing.expectEqualStrings("post-reset failure\n", writer_result.stdout);
...
test "wrapper: raw failure fallback never persists output" {
    const allocator = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const bin_path = try tmp.dir.realPathFileAlloc(std.testing.io, ".", allocator);
...
test "smoke: acli json output is minified" {
    ...
    try std.testing.expectEqual(std.process.Child.Term{ .exited = 0 }, result.term);
    try std.testing.expect(result.stdout.len < fixture.len);
    try std.testing.expectEqualStrings("[{\"key\":\"EXAMPLE-101\",\"summary\":\"Anonymized sample work item\"},{\"key\":\"EXAMPLE-102\",\"summary\":\"Consectetur adipiscing elit\"}]\n", result.stdout);
}
...
test "smoke: failed acli preserves stderr diagnostics" {
    ...
    try writeFakeScript(tmp.dir, "acli",
        \\#!/bin/sh
        \\printf 'error: authentication required for https://example.atlassian.invalid\n' >&2
        \\exit 1
    );

    var result = try runSmllWrapperEnv(allocator, bin_path, &.{ "acli", "jira", "workitem", "search" }, &.{});
    defer result.deinit(allocator);

    try std.testing.expectEqual(std.process.Child.Term{ .exited = 1 }, result.term);
    try std.testing.expectEqualStrings("", result.stdout);
    try std.testing.expectEqualStrings(acli_error_stderr_fixture, result.stderr);
}
